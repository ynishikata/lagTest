...
さらに、複数の役割を持つ AI エージェントが自動的に議論を行う **仮想会議エージェント（マルチエージェント会議）** 機能も含まれています。
...
- **ブラウザUI**
  ...
  - 機能:
    ...
    - 画面下部で会議トピックとラウンド数を指定し、「仮想会議を開始」ボタンから `/meeting` にPOST  
      → 慎重派 / 攻め派 / コンプラ / 現場の4エージェントによる議論ログと、最終サマリを表示
...
- **仮想会議エージェント API**
  - エンドポイント: `POST /meeting`
  - リクエスト:
    - `topic`: 会議トピック（議論してほしいテーマ）
    - `max_rounds` (オプション): 会議ラウンド数（デフォルト 5）
  - 処理内容:
    - 「慎重派AI / 攻め派AI / コンプラAI / 現場AI」の4つのエージェントが、指定ラウンド数だけ交互に発言
    - 各発言は「理由 / 提案 / 質問」を含むフォーマットで生成
    - 全ログをまとめて、最後に「結論・利点・リスク・推奨案」の形式でサマリを生成
  - レスポンス:
    - `topic`: トピック
    - `logs`: 全発言ログ（ラウンド番号・スピーカー名・発言内容）
    - `summary`: 最終サマリ（結論・利点・リスク・推奨案）
...
- **HTTP ハンドラ**
  ...
  - `POST /meeting`
    - 会議トピックとラウンド数を受け取り、複数エージェントでの仮想会議を実行 → ログとサマリを返す。
...
```bash
curl -X POST http://localhost:8080/meeting \
  -H "Content-Type: application/json" \
  -d '{
    "topic": "新しい社内AIアシスタントを導入する際の進め方について議論してください。",
    "max_rounds": 3
  }'
```
